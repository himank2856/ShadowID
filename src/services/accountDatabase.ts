...
import { UserAccount, ScanJob, Role } from '../types.ts';
...
export interface AuditLogEntry {
  id: string;
  userId: string;
  action: 'REGISTRATION' | 'LOGIN_PASSWORD' | 'LOGIN_OTP' | 'OTP_VERIFIED' | 'SCAN_CREATED' | 'DATA_PURGED';
  details: string;
  ipMasked: string;
  timestamp: string;
}
...
export const accountDatabase = {
  ...
  registerUser: async (data: {
    ...
    role: Role;
    password: string;
  }): Promise<{ user: UserAccount; otpTarget: string }> => {
    ...
    // Trigger initial OTP verification to mobile
    otpService.generateAndSendOtp(cleanPhone, 'sms', 'signup');

    logAuditEvent(newUser.id, 'REGISTRATION', `User registered: ${cleanEmail}`);

    return {
      user: newUser,
      otpTarget: cleanPhone,
    };
  },
